...
<form method="get" action="/employees" class="mt-6 grid gap-4 rounded-2xl border border-brand-ink-light bg-brand-panel p-6 sm:grid-cols-2 lg:grid-cols-5">
  ...
  <table class="w-full text-left text-sm">
    <thead class="bg-brand-panel-strong text-slate-400">
      <tr>
        ...
        <th class="px-4 py-3"><?= t('employees.col_code') ?></th>
    ...
    <tbody>
      <?php foreach ($employees as $emp): ?>
        <tr class="border-t border-brand-ink-light">
          ...
          <td class="px-4 py-3"><?= e($emp['employee_code']) ?></td>
      ...
      <?php endforeach; ?>
      <?php if (!$employees): ?>
        <tr><td class="px-4 py-6 text-slate-400" colspan="9">
          <?= array_filter([$filters['date_from'] ?? null, $filters['date_to'] ?? null, $filters['contractor_id'] ?? null, $filters['subcontractor_id'] ?? null, $filters['q'] ?? null])
